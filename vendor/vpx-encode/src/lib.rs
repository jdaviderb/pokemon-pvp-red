...
impl Encoder {
    pub fn new(config: Config) -> Result<Self> {
        ...
        // 1 thread: our frames are tiny (160x144 Game Boy); splitting across 8 threads (the upstream
        // default) just adds sync overhead and floods the scheduler when many encoders run at once.
        c.g_threads = 1;
        ...
        match config.codec {
            VideoCodecId::VP8 => {
                ...
                // CRITICAL for realtime: VP8's default cpu_used is 0 (the SLOWEST, best-quality
                // setting) — far too slow for 60fps and the dominant CPU cost when many battles are
                // watched at once. Positive = faster; 12 is plenty of quality for a 160x144 frame.
